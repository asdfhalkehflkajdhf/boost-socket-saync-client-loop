...
public:
	...
	RPCStruct_ptr read(int reqid){
		...
		std::cout<<"4 cal read stat mapreqid["<<res->head().req_id()<<"] parareqid["<<reqid<<"] status["<<res->get_status()<<std::endl;
		...
		while(true){
			switch(res->get_status()){
				...
				case CT_TIMEOUT:
					...
					std::cout<<"4 cal read ok reqid="<<reqid<<std::endl;
...
private:
    ...
    void handle_read_header(recvHead_ptr head, const boost::system::error_code& error){
        ...
        {
			std::cout<<"6 handle_read_header reqid["<<head->req_id()<<"] | head().body_len["<<head->body_len()<<std::endl;
			//接收map中找到
			if(recv_msg_map_.find(head->req_id()) != recv_msg_map_.end()){
				std::cout<<"6 chun zai"<<std::endl;
				//未超时	 //超时 都需要接收完body内容
                //更新map头部内容
				...
				std::cout<<"6 bu chun zai"<<std::endl;
			//接收map中未找到
        ...
        {
            std::cout<<"6 cal handle_read_header ["<<error_msg(error.value())<<"]"<<std::endl;
            do_close();
        }
    ...
    void do_read_msg(RPCStruct_ptr read_msg_){
		...
		std::cout<<"7 need read body_len "<<read_msg_->head().body_len()<<std::endl;
        boost::asio::async_read(*socket_,
    ...
    void handle_read_msg(RPCStruct_ptr read_msg_, const boost::system::error_code& error){
        ...
        {
			//第一部分，处理接收回调函数
            std::cout<<"8 cal handle_read_msg recv | reqid= "<<read_msg_->head().req_id()<<" | body= ["<<read_msg_->body()<<"]"<<std::endl;
            //更新读取状态；
        ...
        else
        {
            ...
            std::cout<<"8 cal handle_read_msg "<<" ["<<error_msg(error.value())<<"]"<<std::endl;
            do_close();
        }
    }
    ...
    void handle_write(const boost::system::error_code& error)
    {
        ...
        {
            std::cout<<"3 cal handle_write "<<" ["<<error_msg(error.value())<<"]"<<std::endl;
            do_close();
        }
    }
    //完成发送消息队列和接收消息map新建
    ...
    void erase_msg_timer(){
        //std::cout<<"cal  erase_msg_timer delSize"<<del_reqid_list.size()<<" recvSize"<<recv_msg_map_.size()<<std::endl;
